...
class GetRepoInteractor @Inject constructor(private val repoApiService: RepoApiService,
  ...
  /**
   * Defining the url for the first page request.
   *
   * @param url The base url of a GitHub user.
   */
  ...
  /**
   * Request the next page, using the LinkHeader provided by GitHub.
   *
   ...
   *
   * Important: Cache do not have next page.
   *
  ...
  inner class GetRepoObserver: Observer<Response<ResponseData>> {
    ...
    override fun onNext(result: Response<ResponseData>) {
      ...
      if (subscription.hasObservers()) {
        ...
        // Parsing and updating the LinkHeader.
        result.headers().let {
          it.get("Link")?.let {
  ...
  /**
   * Retrieve all the Repos saved into Realm.
   */
